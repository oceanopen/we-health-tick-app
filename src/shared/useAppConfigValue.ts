 ...
 *
 * @param key 配置项 key（与 src/shared/appConfig.ts 的 *_KEY 常量对齐）
 ...
 */
export function useAppConfigValue<T>(
  key: string,
  ...
  useEffect(() => {
    ...
    getAppConfig(key)
      .then(apply)
      .catch((e: unknown) => {
        console.warn(`[useAppConfigValue:${key}] load failed`, e);
      });
    ...
    return () => {
      unlisten
        .then(fn => fn())
        .catch((err: unknown) => console.warn(`[useAppConfigValue:${key}] unlisten failed:`, err));
    };
  }, [key, decode]);
