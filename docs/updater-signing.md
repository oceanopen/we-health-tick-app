...
同时 `bundle.createUpdaterArtifacts: true` 让 bundler 额外产出 `.tar.gz` + `.sig`（自动更新必需），与 `targets: ["dmg", "nsis"]` 共存。
...
- **关键步骤**：`tauri-apps/tauri-action@v0`，仅当 `github.ref_type == 'tag'` 时执行 release：
  - 注入 `TAURI_SIGNING_PRIVATE_KEY` → 给 `.app.tar.gz` / nsis `.tar.gz` 签名生成 `.sig`
  - 生成 `latest.json` manifest（含版本、下载 URL、签名）
  - 上传 dmg / exe / `*.tar.gz` / `*.sig` / `latest.json` 到 GitHub Release
- `updaterJsonPreferNsis: true`：与 `bundle.targets=["nsis"]` 对齐，Windows 更新走 nsis 包。
...
- `We Health Tick_<ver>_x64-setup.exe` / `nsis .tar.gz` / `nsis .tar.gz.sig`（Windows）
- `latest.json`
